...
|---|---|---|---|
| 0 | Installable package, CLI, zero-telemetry posture, name/license decision | `pyproject.toml`, `LICENSE`, CI/build and wheel scans | Implemented and locally verified. MIT is confirmed and declared with SPDX/PEP 639 metadata. The exact PyPI name remains unreserved until the pending publisher performs its first upload. |
...
- Enable PyPI 2FA, configure the pending Trusted Publishers and protected GitHub environments in `docs/PUBLISHING.md`, then publish and independently install 0.1.1.
- Review and explicitly run the desired service installer; setup intentionally leaves this human-controlled.
